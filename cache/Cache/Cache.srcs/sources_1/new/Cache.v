...
input clk,                      //clock
input rst,                      //reset
input[31:0] data_cpu_wt,        //data write in
input[127:0] data_mem_rd,       //data read in
input[31:0] addr_cpu,           //cpu addr
input wt_cpu,                   //cpu write enable
input rd_cpu,                   //cpu read enable
input mem_ready,                //memory ready
//output
output reg wt_mem,              //memory write enable
output reg rd_mem,              //memory read enable
output reg[127:0] data_mem_wt,  //data to mem
output reg[31:0] data_cpu_rd    //data to cpu
);
 ...
 assign cpu_req_valid = rd_cpu | wt_cpu;//Valid CPU request
 assign tag_out0 = tag_out_orig0[22:0];
 assign tag_out1 = tag_out_orig1[22:0];
 assign cpu_tag = addr_cpu[31:9];//tag width
 assign cpu_idx = addr_cpu[8:2];//index width
 assign cpu_offset = addr_cpu[1:0];//word width
...
if(mem_ready) begin
 next_state<=2'b11;
 wt_mem<=1;
data_mem_wt<=tag_out_orig1[23]?dout0:dout1;//LRU
  end
else begin
next_state<=2'b10;
end
